...
**Current coverage:** 52 published playbooks · 30 working files · 43 curated tools · 19 reading sources · 9 domain guides · 8 domains with published playbooks
...
- What is going on in this named account, and what will we do in 30/60/90 days?
- How should people, process, and performance run the book after the sale?
- How should first value be educated and implemented without mixing the two jobs?
- What must a new customer receive in writing before and after kickoff?
- What should a new CS executive actually do in 90 days?
- Which post-sale fields must exist so CS can see value, cadence, renewal, and risk?
- Can leadership defend the path to the number, or is the forecast theater?
- What must a demo prove after the pitch, and how do we score it rather than a vibe?
- When does fit and behavior justify a different action this week?
- Can demand creation and sales capacity produce the same number?
...
- Which GTM assumption will we try to disprove this quarter?
- How should we show up in search and AI answers without making clicks the only scoreboard?
- Where does each GTM motion sit on the AI ladder, and which rung do we climb next?
- Which company problem deserves an AI bet this quarter—not which tool?
- What artifact and gate must exist before we build an AI workflow?
...
- Do market, product, channel, and model still describe the same company?
- Has this outbound sentence already worked by hand—before we automate it?
- When should the launch sit if it must not land on the close?
...
7. [Four Fits](playbooks/01-strategy-and-buyers/four-fits.md) — whether market, product, channel, and model are still the same company
8. [Positioning](playbooks/02-product-marketing/positioning.md) — why they should change from the current alternative
9. [Sales enablement](playbooks/02-product-marketing/sales-enablement.md) — how that sounds in the first meeting
10. [Pricing and packaging](playbooks/02-product-marketing/pricing-and-packaging.md) — what they pay, and when you ask
11. [Content strategy](playbooks/03-brand-story-and-content/content-strategy.md) — which pages a champion can forward
12. [Channel strategy](playbooks/04-channels-and-distribution/channel-strategy.md) — which motion actually creates customers
...
If you already have a sales motion and named accounts, the living file is [account planning](playbooks/06-account-field-and-partner/account-planning.md). After they buy, [customer success](playbooks/08-lifecycle-and-customer-marketing/customer-success.md) is the book, [customer onboarding](playbooks/08-lifecycle-and-customer-marketing/customer-onboarding.md) is first value, and [onboarding communication](playbooks/08-lifecycle-and-customer-marketing/onboarding-communication.md) is what they receive in writing. A new CS executive’s 90 days are [CS-leadership ramp](playbooks/08-lifecycle-and-customer-marketing/cs-leadership-ramp.md). The post-sale record is [CS workspace](playbooks/08-lifecycle-and-customer-marketing/cs-workspace.md). Whether the quarter’s number is real is [forecasting](playbooks/09-operations-pipeline-and-measurement/forecasting.md). Whether next year’s number is even possible is [GTM planning](playbooks/09-operations-pipeline-and-measurement/gtm-planning.md). None of these replace the thirteen chapters above.
...
| **Working files** | Open a spreadsheet, scorecard, or 90-day outline in Google Sheets/Docs/Slides | [Browse the 30 working files](TEMPLATES.md) |
| **Tools** | Find AI-native and modern B2B software for a defined operating job | [Browse the 43-tool directory](TOOLS.md) |
| **Reading sources** | Find newsletters, podcasts, and operators worth following | [Browse the 19-source directory](RESOURCES.md) |
...
| 05 | [Outbound & prospecting](playbooks/05-outbound-and-prospecting/) | How to create relevant conversations with accounts that have not raised a hand | [Account research](playbooks/05-outbound-and-prospecting/account-research.md), [buying signals](playbooks/05-outbound-and-prospecting/buying-signals.md), [message-market fit](playbooks/05-outbound-and-prospecting/message-market-fit.md), [cold email](playbooks/05-outbound-and-prospecting/cold-email.md), [multichannel sequence](playbooks/05-outbound-and-prospecting/multichannel-sequence.md), [SDR onboarding](playbooks/05-outbound-and-prospecting/sdr-onboarding.md), [contact data](playbooks/05-outbound-and-prospecting/contact-data.md); planned: cold call, LinkedIn outbound |
...
Planned topics remain in the master index until the page is complete. The repository does not publish empty placeholders, fictional examples, or anonymous “case studies” presented as evidence. Templates and checklists stay on every page so an agent can execute; they are not a substitute for a chapter you can read. When the job is a living spreadsheet or a 90-day deck, the blank lives in [TEMPLATES.md](TEMPLATES.md) so you can open it in Google Sheets, Docs, or Slides. Named companies that appear inside playbooks are **attributed illustrations** from dated sources (founder interviews, practitioner essays). They are not the planned [Verified use cases](#start-with-one-decision) collection, which will only publish a company when this repository can point to a primary source and a date for *that* motion.
...
- The library grows one complete tactic playbook at a time, following each domain's recommended build order unless a live operating gap jumps the queue.
...
**当前覆盖：** 52 篇已发布 Playbook · 30 份工作文件 · 43 个精选工具 · 19 个阅读源 · 9 个领域指南 · 8 个领域已有正式 Playbook
...
- 这个具名账户里发生了什么，未来 30/60/90 天做什么？
- 成交后这本书该怎么用人、流程、绩效来管？
- 客户首次价值该怎么教育和落地，而不是把两件事混成一件？
- 新客户在 kickoff 前后必须拿到哪些书面材料？
- 新来的 CS 负责人前 90 天到底该干什么？
- 成交后哪些字段必须存在，CS 才能看见价值、节奏、续约和风险？
- 通往那个数字的路径能不能被辩护，还是预测只是演戏？
- Demo 在讲完定位之后必须证明什么，怎么打分而不是凭感觉？
- 匹配度和行为什么时候够得上换一种动作？
- 需求端能造出来的、销售端能关得上的，是不是同一个数字？
...
- 这个季度我们准备证伪哪一条 GTM 假设？
- 搜索和 AI 回答里怎么被看见，而又不把点击当成唯一分数？
- 每条 GTM 动作现在处在 AI 梯子的哪一级，下一步只升一级？
- 这个季度该押哪一个公司问题，而不是哪一个 AI 工具？
- 开工前必须写清哪个产物、哪道门闩，才能建 AI 工作流？
- 拨打之前怎么确认这个号码是本人？
...
- 扫完胸卡之后，用什么证据给跟进排序？
- 什么样的内容分发线索才准进猎人队列？
- 市场、产品、渠道、模式是否还是同一家公司？
- 这句外联在人工小样本上是否已经跑通——再谈自动化？
- 发布日该落在哪一周，才不会砸在季度收官上？
...
7. [四重契合](playbooks/01-strategy-and-buyers/four-fits.md) — 市场、产品、渠道、模式是否还是同一家公司
8. [定位](playbooks/02-product-marketing/positioning.md) — 为什么他们该离开现状
9. [销售赋能](playbooks/02-product-marketing/sales-enablement.md) — 第一次会上怎么讲
10. [定价与包装](playbooks/02-product-marketing/pricing-and-packaging.md) — 收多少、何时开口
11. [内容策略](playbooks/03-brand-story-and-content/content-strategy.md) — 冠军买家能转发哪些页面
12. [渠道策略](playbooks/04-channels-and-distribution/channel-strategy.md) — 真正制造客户的是哪条动作
...
如果你已经有销售动作和具名账户，活文件是 [账户规划](playbooks/06-account-field-and-partner/account-planning.md)。成交之后，[客户成功](playbooks/08-lifecycle-and-customer-marketing/customer-success.md) 管这本书，[客户入职](playbooks/08-lifecycle-and-customer-marketing/customer-onboarding.md) 管首次价值，[入职沟通](playbooks/08-lifecycle-and-customer-marketing/onboarding-communication.md) 管他们拿到的书面材料。新来的 CS 负责人前 90 天见 [CS 负责人 90 天](playbooks/08-lifecycle-and-customer-marketing/cs-leadership-ramp.md)。成交后的记录见 [CS 工作区](playbooks/08-lifecycle-and-customer-marketing/cs-workspace.md)。季度数字是否真实，看 [预测](playbooks/09-operations-pipeline-and-measurement/forecasting.md)。明年那个数字是否可能，看 [GTM 规划](playbooks/09-operations-pipeline-and-measurement/gtm-planning.md)。这些都不能代替上面的十三章。
...
| **Working files 工作文件** | 把记分表、表格或 90 天大纲放到 Google 表格/文档/幻灯片里用 | [浏览 30 份工作文件](TEMPLATES.md) |
| **Tools 工具目录** | 围绕明确任务查找 AI-native 和新一代 B2B 软件 | [浏览包含 43 个产品的工具目录](TOOLS.md) |
| **Reading sources 阅读源** | 查找值得长期关注的 Newsletter、播客和运营者 | [浏览包含 19 个来源的目录](RESOURCES.md) |
...
5. 可复制的模板；当工作是表格、记分表或 90 天大纲时，另有 [工作文件](TEMPLATES.md)；
6. 执行前检查清单；
7. 运营指标与不应误用的指标；
8. 常见错误；
9. 来源、证据边界、最后核验日期，以及下一章指向。

计划中的主题会继续保留在总索引里，直到页面达到完整发布标准。仓库不会发布空占位页面、虚构案例，也不会把匿名“案例研究”当成事实证据。模板和清单留在每页上，是为了让智能体也能执行；它们不能代替一篇能读下去的章节。当工作是活表格或 90 天大纲时，空白文件在 [TEMPLATES.md](TEMPLATES.md)，可以导入 Google 表格、文档或幻灯片。Playbook 正文里出现的具名公司，是带日期来源的**访谈/文章插图**，不是计划中的 [已验证 Use Cases](#从一个决策开始) 专栏。专栏只会在本仓库能指出该动作的一手来源和日期时才发布。
...
- 知识库按「每天一篇完整战术 Playbook」增长：默认跟随各领域建议顺序，除非当前真实卡点需要插队。
