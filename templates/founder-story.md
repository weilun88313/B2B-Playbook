---
title: "Founder-story card"
sidebarTitle: "Founder Story"
---

B2B Playbook working file. Read [founder story](../playbooks/03-brand-story-and-content/founder-story.md) first.

One person. One perception. Delete teaching notes. Do not ghostwrite a voice they will not say on a call.

Copyright © 2026 Ivan Xu. Private operating copy permitted. See LICENSE.

---

![Perception; Scene; Advantage check](../assets/illustrations/worksheet-founder-story.webp)

## Perception

- Statement in the buyer’s mouth:
- Why only this person can prove it:

## Scene

- Job they had:
- What broke:
- What they refuse to sell as the fix:
- Why the ICP should care:

## Advantage check

- They already have the room? yes / no / evidence:
- They will show up on a cadence we can staff? yes / no / what they will actually do:
- If either is no, we will not call this a marketing advantage.

## Surfaces (exactly two this quarter)

1.
2.

- What we will not claim:
- GACCS owner:
