---
title: "Executive-dinner table card"
sidebarTitle: "Executive Dinners"
---

B2B Playbook working file. Read [executive dinners](../playbooks/06-account-field-and-partner/executive-dinners.md) first.

One table. One seat. One question. Delete teaching notes.

Copyright © 2026 Ivan Xu. Private operating copy permitted. See LICENSE.

---

![Night; Guests (8–12); Rules](../assets/illustrations/worksheet-executive-dinners.svg)

## Night

- Date / city / host:
- Seat lock (one):
- Question (one interrogative):

## Guests (8–12)

| Name | Account | Why them | Customer or peer? |
|---|---|---|---|
| | | | |

## Rules

- What we will not do (demo, extra titles, recap blast):
- Capture fields:
- Next-morning owner:

## After

- Qualified next actions:
- Mixer titles we added (must be none):
