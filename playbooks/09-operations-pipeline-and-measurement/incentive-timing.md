...
| Word | Meaning here |
|---|---|
| **Clawback** | Money already paid is taken back when a written trigger hits inside a window |
| **Holdback** | A slice of variable is not paid until cash, go-live, or another milestone |
| **Recoverable draw** | Advance against future commissions; the balance is recouped |
...
A policy that can be administered:
...
Guardrails that keep draws from becoming pay:
...
| Field | Fill |
|---|---|
| Credit | Counts on countersigned order (same as the compensation page). |
| Payout | 80% of variable on that deal at credit. 20% holdback until first invoice collected or 45 days, whichever first. |
| Clawback | If the customer cancels or never pays inside **90 days** of credit, recover the paid slice from future commissions. Cap: 40% of that period’s variable. Quota credit stays. |
| Visibility | CRM field: window end date. Payroll statement names the deal. |
| New-hire draw | Months 1–3: recoverable advance equal to 80% of target monthly variable. Commissions offset the balance; recoup at most 40% of earned commissions per period. Then stop. |
| Refused | Team-wide draw because Q2 missed. That is a quota/territory conversation. |

Copy the empty fields. Do not copy the 90 days or the 80/20 split as market law.
...
|---|---|
| Clawback dollars / commissions paid | Whether pay is ahead of revenue |
| Disputes on recovery | Unwritten triggers |
| Draw exception count and aging balances | Timing bridge vs subsidy |
| Windows still open as a % of credited deals | Visibility |
| Mid-year improvised recoveries | Process failure |
...
Clawback as a designed mechanism (triggers, window, recovery from future pay, visibility, cash-posture), holdback as an alternative to paying the risky slice, and draws as time-boxed exceptions (recoverable vs company-caused non-recoverable, no stacking, monthly exception count) are distilled from public RevOps/finance reports on clawback policy and sales draws ([QuotaPath clawback playbook](https://www.quotapath.com/resource/clawback-policy-playbook/?ref=b2b-playbook); [QuotaPath sales draws](https://www.quotapath.com/resource/sales-draws-report/?ref=b2b-playbook)). Those pages are **method prompts**, not sources to copy. Conversation counts, percentages, payout-mix tables, role grids, and product pitches in those reports are **not** this library’s facts or a requirement to buy their software. Named customer anecdotes in the reports stay theirs.
